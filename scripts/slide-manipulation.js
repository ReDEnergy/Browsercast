...
function reloadSlides() {
	...
	var nr_slides = getNumberOfSlides();
	if (nr_slides >= 2)
		setDeleteEvent();
	if (nr_slides === 1)
		unsetDeleteEvent();
...
function getNumberOfSlides() {
	...
	Array.prototype.forEach.call(slides.children, function(slide) {
		if (slide.tagName === 'SECTION') {
			if (slide.classList.contains('stack')) {
				Array.prototype.forEach.call(slide.children, function (s) {
	...
	console.log('NR OF SLIDES: ', count);
	return count;
}
...
delete_confirm.addEventListener('click', stopPropagation);
